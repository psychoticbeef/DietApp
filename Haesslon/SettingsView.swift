...
// MARK: - FillerFoodDetailView Update
...
struct FillerFoodDetailView: View {
    ...
    var body: some View {
        Form {
            // --- NEW SECTION: SMART FILL ---
            Section(header: Text("Smart Fill")) {
                HStack(spacing: 20) {
                    Button {
                        showScanner = true
                    } label: {
                    ...
                    .buttonStyle(.borderless) // Important in Forms
                    
                    Divider()
                    
                    Button {
                        if let string = UIPasteboard.general.string {
                            applyParsedData(from: string)
                        } else {
        ...
        .sheet(isPresented: $showScanner) {
            ScannerSheet { scannedText in
                applyParsedData(from: scannedText)
            }
    ...
    // Logic to apply the parsed result
    private func applyParsedData(from text: String) {
        let result = NutritionParser.parse(text)
        
        // Haptic feedback
        let generator = UINotificationFeedbackGenerator()
        generator.notificationOccurred(result.hasData ? .success : .error)
        
        withAnimation {
            if let v = result.kcal { food.kcalPer100g = v }
            if let v = result.fat { food.fatPer100g = v }
            if let v = result.satFat { food.satFatPer100g = v }
            if let v = result.carbs { food.carbsPer100g = v }
            if let v = result.sugar { food.sugarPer100g = v }
            if let v = result.protein { food.proteinPer100g = v }
            if let v = result.fiber { food.fiberPer100g = v }
            
            // Convert Salt (g) to Sodium (mg)
            // Sodium = Salt / 2.5 * 1000
            if let saltGrams = result.salt {
                food.sodiumPer100g = (saltGrams / 2.5) * 1000
            }
        }
    }
    
    // ... (Existing focus helpers: moveFocus, canMoveBack, etc.) ...
    private func moveFocus(forward: Bool) {
        guard let current = focusedField,
              let index = fieldOrder.firstIndex(of: current) else { return }
